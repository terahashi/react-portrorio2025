...
const initThree = (container) => {
  // ⬆︎引数(container)は「#dog-area」
  // containerは「3d.jsxから渡ってきた initThree(containerRef.current); 」 = #dog-areaに設定したref
  //「 getBoundingClientRect で「#dog-areaの表示されている幅・高さ」を取得するもの。」
  const rect = container.getBoundingClientRect(); //containerは「#dog-area」
  ...
  container.appendChild(renderer.domElement); //「dog-area」にレンダラーのDOM要素を追加
  ...
  loader.load(
    ...
    (gltf) => {
      ...
      dog.scale.set(0.5, 0.5, 0.5); // ⭕️(ここ大事)3Dモデルの大きさを調整
      scene.add(dog); // シーン(scene)に3Dモデルを追加
  ...
  // ======================
  const handleResize = () => {
    const rect = container.getBoundingClientRect(); //'resize'でウィンドウのサイズが変わったときに「"getBoundingClientRect"で、container(#dog-Area)の「表示されている幅・高さ」を取得するもの。」
    renderer.setSize(rect.width, rect.height); // setSizeで描画サイズを「rect「dog-area」の表示サイズにする」
